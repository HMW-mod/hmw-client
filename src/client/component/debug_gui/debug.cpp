...
namespace gui::debug
{
	namespace
	{
		...
		bool world_pos_to_screen_pos(float* origin, float* out)
		{
			...
			out[0] = (width / 2) * (1 - transform[0] / refdef->fovX / transform[2]);
			out[1] = (height / 2) * (1 - transform[1] / refdef->fovY / transform[2]);
		...
		void update_camera()
		{
			...
			camera[0] = refdef->org[0];
			camera[1] = refdef->org[1];
			camera[2] = refdef->org[2];

			axis[0][0] = refdef->axis[0][0];
			axis[0][1] = refdef->axis[0][1];
			axis[0][2] = refdef->axis[0][2];

			axis[1][0] = refdef->axis[1][0];
			axis[1][1] = refdef->axis[1][1];
			axis[1][2] = refdef->axis[1][2];

			axis[2][0] = refdef->axis[2][0];
			axis[2][1] = refdef->axis[2][1];
			axis[2][2] = refdef->axis[2][2];
