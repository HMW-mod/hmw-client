...
namespace fastfiles
{
	...
	namespace
	{
		...
		void db_try_load_x_file_internal(const char* zone_name, const int flags)
		{
			...
			static std::vector<std::string> zone_ignore = {
				{ "common_depot_mp", "cache_ui_mp", "cache_code_post_gfx_mp", "eng_cache_ui_mp", "cache_common_mp",
				  "eng_cache_code_post_gfx_mp", "eng_cache_mp_vlobby_room", "cache_mp_vlobby_room_path", "eng_cache_mp_vlobby_room_path" }
			};

			if (std::find(zone_ignore.begin(), zone_ignore.end(), zone_name) != zone_ignore.end())
			{
#ifdef DEBUG
				console::debug("Ignoring fastfile \"%s\"\n", zone_name);
#endif
				return;
			}

			const auto override_zone_name = zone_overrides.find(zone_name);
			if (override_zone_name != zone_overrides.end())
			{
#ifdef DEBUG
				console::debug("Overriding fastfile \"%s\" with \"%s\"\n", zone_name, override_zone_name->second.c_str());
#endif
				zone_name = override_zone_name->second.c_str();
			}
			...
			{
#ifdef DEBUG
				//console::error("Couldn't load fastfile '%s'\n", zone_name);
#endif
			}
		}
	...
	public:
		void post_unpack() override
		{
...
#ifdef DEBUG
			const auto print_assets_flag = game::DVAR_FLAG_SAVED;
#else
			const auto print_assets_flag = game::DVAR_FLAG_READ;
#endif
			db_print_default_assets = dvars::register_bool("db_printDefaultAssets",
				false, print_assets_flag, "Print default asset usage");
			...
			if (game::environment::is_mp())
			{
				...
				utils::hook::nop(0x3981B5_b, 5); // com_sprintf cache_
				utils::hook::nop(0x3981A3_b, 8);
				utils::hook::nop(0x3981C1_b, 2);
				utils::hook::nop(0x3981C3_b, 4);
				utils::hook::nop(0x3981C7_b, 8);


				utils::hook::nop(0x3982C1_b, 5); // com_sprintf %s_cache_
				utils::hook::nop(0x3982CD_b, 2);
				utils::hook::nop(0x3982CF_b, 4);
				utils::hook::nop(0x3982D3_b, 8);
				utils::hook::nop(0x39847F_b, 5); // com_sprintf %s_cache_
				utils::hook::nop(0x398484_b, 2);
				utils::hook::nop(0x398486_b, 5);
				utils::hook::nop(0x39848B_b, 5);
			}
