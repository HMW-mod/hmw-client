...
#define USE_CUSTOM_STYLE                    true		// If true, a custom style will be used
...
public:
    ...
    inline const char* getIcon() 
    {
        ...
        case ImGuiToastType::None:
            return nullptr;
        case ImGuiToastType::Success:
            return reinterpret_cast<const char*>(u8"\uf058"); // Font Awesome 6
        case ImGuiToastType::Warning:
            return reinterpret_cast<const char*>(u8"\uf06a"); // Font Awesome 6
        case ImGuiToastType::Error:
            return reinterpret_cast<const char*>(u8"\uf057"); // Font Awesome 6
        case ImGuiToastType::Info:
            return reinterpret_cast<const char*>(u8"\uf05a"); // Font Awesome 
        case ImGuiToastType::Debug:
            return reinterpret_cast<const char*>(u8"\uf188"); // Font Awesome 6
        default:
            return nullptr;
...
namespace ImGui
{
    ...
    inline void RenderNotifications()
    {
      ...
      /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      // This holds our custom styling for our notifications
        #if USE_CUSTOM_STYLE
            float slideInProgress = std::min(1.0f, std::chrono::duration<float>(currentToast->getElapsedTime()).count() / 0.5f);
            float slideInOffset = (1.0f - slideInProgress) * 300.0f;

            const int numStyleVars = 2;

            // Custom styling
            PushStyleVar(ImGuiStyleVar_WindowRounding, 10.0f);
            PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(15.0f, 15.0f));

            // Calculate content size before rendering
            float maxWidth = 400.0f;
            float minWidth = 300.0f;
            float padding = 30.0f; // Total horizontal padding

            // Calculate text widths without wrapping for initial size estimate
            float titleWidth = 0.0f;
            if (!NOTIFY_NULL_OR_EMPTY(title)) {
                titleWidth = CalcTextSize(title).x;
            }
            else if (!NOTIFY_NULL_OR_EMPTY(defaultTitle)) {
                titleWidth = CalcTextSize(defaultTitle).x;
            }

            // Add icon width if present
            if (!NOTIFY_NULL_OR_EMPTY(icon)) {
                titleWidth += CalcTextSize(icon).x + GetStyle().ItemSpacing.x;
            }

            // Calculate content width with proper wrapping
            float wrapWidth = maxWidth - padding;
            float textWidth = 0.0f;
            if (!NOTIFY_NULL_OR_EMPTY(content)) {
                // Split content into words and calculate optimal width
                std::string contentStr(content);
                std::istringstream iss(contentStr);
                std::string word;
                float lineWidth = 0.0f;
                while (iss >> word) {
                    float wordWidth = CalcTextSize(word.c_str()).x;
                    if (lineWidth + wordWidth > wrapWidth) {
                        textWidth = std::max(textWidth, lineWidth);
                        lineWidth = wordWidth;
                    }
                    else {
                        lineWidth += wordWidth + CalcTextSize(" ").x;
                    }
                }
                textWidth = std::max(textWidth, lineWidth);
            }

            // Calculate final window width
            float windowWidth = std::max(minWidth, std::min(maxWidth, std::max(textWidth, titleWidth) + padding));

            // Set window size
            SetNextWindowSize(ImVec2(windowWidth, 0.0f));

            // Calculate position with slide animation
            ImVec2 mainWindowPos = GetMainViewport()->Pos;
            SetNextWindowPos(
                ImVec2(
                    mainWindowPos.x + mainWindowSize.x - NOTIFY_PADDING_X - slideInOffset,
                    mainWindowPos.y + NOTIFY_PADDING_Y + height
                ),
                ImGuiCond_Always,
                ImVec2(1.0f, 0.0f)
            );
            ...
            SetNextWindowPos(
                ImVec2(
                    mainWindowPos.x + mainWindowSize.x - NOTIFY_PADDING_X,
            ...
            {
                // Set text wrapping just once, using our calculated width
#if USE_CUSTOM_STYLE
                PushTextWrapPos(GetCursorPos().x + windowWidth - padding);
#else
                ...
                if (NOTIFY_USE_DISMISS_BUTTON)
                {
                    ...
                    SetCursorPosX(GetWindowWidth() - GetStyle().WindowPadding.x - CalcTextSize("X").x - 5.0f);
